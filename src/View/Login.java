...
public class Login extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == btnLogin) {
            ...
            if (acc != null) { // Kiểm tra xem acc có khác null không
                String role = acc.getRole();

                if ("admin".equals(role)) { // Đảm bảo kiểm tra role tránh null
                    adminView.showView();
                    dispose();
                } else if ("user".equals(role)) { // Đảm bảo kiểm tra role tránh null
